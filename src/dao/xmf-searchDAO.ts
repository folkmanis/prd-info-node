...
export class XmfSearchDao extends Dao {
  ...
  async findJobs(
    search: ArchiveSearchParams,
    userPreferences: UserPreferences,
  ): Promise<XmfArchiveResponse>;
  async findJobs(
    search: ArchiveSearchParams,
    userPreferences: UserPreferences,
    start: string,
  ...
  async findJobs(
    ...
    const facet = (
      await this.archSorted.aggregate<FacetResult>(pipeline).toArray()
    )[0];
    return {
      error: false,
      count,
  ...
  private getFilter(
    ...
    filter.CustomerName = {
      $in: search.customerName ? search.customerName : customers,
    };
    if (search.q) {
      filter['$or'] = [
        { JDFJobID: search.q },
