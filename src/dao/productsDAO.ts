...
export class ProductsDao extends Dao {
  ...
  async injectDb(db: Db): Promise<void> {
    ...
    try {
      ...
      await this.updateDb().then((_) => this.createIndexes());
    } catch (err) {
  ...
  async getCustomerProducts(customerName: string): Promise<ProductResult> {
    ...
    try {
      const customerProducts = await this.products
  ...
  async updateProduct(name: string, prod: ProductNoId): Promise<ProductResult> {
    if (prod.productionStages) {
      prod.productionStages = prod.productionStages.map(this.mapProdStage);
    }
  ...
  async touchProduct(customer: string, data: string[]): Promise<ProductResult> {
    ...
    try {
      const resp = await this.products.updateMany(filter, update, {
        writeConcern: { w: 0 },
      });
      return {
        error: false,
    ...
    return this.products
      ...
      .then((result) => {
        if (result.modifiedCount > 0) {
          Logger.info(
            `Products Collection updated. ${result.modifiedCount} records set units to '${DEFAULT_UNIT}'`,
          );
        }
        return;
      });
