import { MongoClient, Collection, ObjectId, BulkWriteOperation, BulkWriteOpResultObject, Db } from "mongodb";
...
describe('create counters collection', () => {
    ...
    it('should find missing records', () => {
        expect(
            countersDAO.filterMissingRecords(defaultValues, existing)
        ).toEqual(missing);
    });
    ...
    it('should create db instructions for missing items', () => {
        ...
        expect(
            countersDAO.createInsertForMissingRecords(records)
        ).toEqual(missingsUpdate);
