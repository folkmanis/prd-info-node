...
export class ProductionStagesDao
  extends Dao
  implements EntityDao<ProductionStage>
{
  ...
  async injectDb(db: Db): Promise<void> {
    try {
      this.productionStage = db.collection(
        this.PRODUCTIONSTAGE_COLLECTION_NAME,
      );
    } catch (err: any) {
  ...
  ): Promise<InsertOneWriteOpResult<ProductionStage>> {
  ...
  updateOne(
    id: string,
    entity: Partial<ProductionStage>,
  ...
  deleteOneById(id: string): Promise<DeleteWriteOpResultObject> {
    return this.productionStage.deleteOne({ _id: new ObjectId(id) });
  }

  async validationData<K extends keyof ProductionStage>(
    key: K,
  ): Promise<Array<ProductionStage[K]>> {
    const resp = await this.productionStage
      .find()
      .project({ _id: 0, [key]: 1 })
      .toArray();
