...
export class JobsDao extends Dao {
  ...
  async getKastesJobs(veikali = false): Promise<KastesJobResponse> {
    const pipeline = [
    ...
    try {
      const result = await this.jobs
        .aggregate<KastesJobPartial>(pipeline)
        .toArray();
      return {
        error: false,
        data: result,
  ...
  async insertJob(job: Job): Promise<Job> {
    ...
    const result = this.jobs
      .insertOne(job)
      .then((result) => this.jobs.findOne({ _id: result.insertedId }))
      .then((newJob) => {
        if (!newJob) {
          throw 'Job insert failed';
        }
        return newJob;
      });
    return result;
  ...
  async insertJobs(insertJobs: Job[]): Promise<JobResponse> {
    ...
    if (!(insertJobs && insertJobs.length > 0)) {
      return { error: null, insertedCount: 0 };
    }
    insertJobs.forEach((job) => (job = this.validateJob(job)));
    try {
      const { insertedCount, insertedIds } = await this.jobs.insertMany(
        insertJobs,
      );
      return {
        error: false,
  ...
  async updateJobs(jobsUpdate: Partial<Job>[]): Promise<number> {
    const operations: BulkWriteUpdateOneOperation<Job>[] = jobsUpdate.map(
      (job) => ({
    ...
    return await this.jobs
      ...
      .then((result) => {
        if (result.modifiedCount > 0) {
          Logger.info(
            `Updated ${result.modifiedCount} jobs status to jobStatus.generalStatus: 20`,
          );
        }
      })
      .then((_) =>
      ...
      .then((result) => {
        if (result.modifiedCount > 0) {
          Logger.info(
            `Updated ${result.modifiedCount} jobs status to jobStatus.generalStatus: 50`,
          );
        }
      })
      .then((_) =>
      ...
      .then((result) => {
        if (result.modifiedCount > 0) {
          Logger.info(
            `Updated ${result.modifiedCount} jobs products.units to '${DEFAULT_UNIT}'`,
            result,
          );
        }
      });
  ...
  jobUpdate(job0: Partial<Job>): BulkWriteUpdateOperation<Job> {
    ...
    if (job.products && !(job.products instanceof Array)) {
      ...
      Object.assign(
        job,
        ...(Object.keys(products) as Array<keyof JobProduct>).map((key) => ({
          [`products.${idx}.${key}`]: products[key],
        })),
      );
      delete job.products;
    }
