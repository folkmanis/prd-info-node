...
export class CustomersDao extends Dao {
  ...
  async getCustomer(idOrName: string): Promise<Customer | undefined> {
    const fltr = /^[a-f\d]{24}$/i.test(idOrName)
  ...
  async insertCustomer(customer: Customer): Promise<CustomerResult> {
    try {
      if (!customer.CustomerName) {
        throw new Error('CustomerName not provided');
      }
      const insertResult = await this.customers
        .findOneAndDelete({ CustomerName: customer.CustomerName })
