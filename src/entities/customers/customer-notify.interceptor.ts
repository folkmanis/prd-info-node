...
export class CustomerNotifyInterceptor implements NestInterceptor {
  ...
  private notify = () => this.notifications.notify(new SystemNotification({ operation: Systemoperations.MESSAGES_UPDATED }));
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<Customer> {
    ...
    return of(params.id).pipe(
      ...
      mergeMap(old => next.handle().pipe(
        mergeMap(upd => this.notifyMessages(old).pipe(
          mergeMap(result => result ? of(false) : this.notifyMessages(upd)),
          mapTo(upd),
        ))
      ))
    );
  ...
  private notifyMessages(customer: Customer): Observable<boolean> {
    ...
    return !folder ? of(false) : this.messages.ftpFolderUploads(folder).pipe(
  ...
  private isUpdate(upd: UpdateCustomerDto): boolean {
    return typeof upd.ftpUser === 'boolean' || typeof upd.ftpUserData?.folder === 'string' || typeof upd.disabled === 'boolean';
  }
