...
export class InvoicesDao {
    ...
    constructor(
        ...
        this.collection = this.dbService.db().collection(this.INVOICES_COLLECTION_NAME);
        this.createCollection(dbService.db())
    ...
    async getOne(invoiceId: string): Promise<InvoiceForReport> {
        ...
        const result = await this.collection.aggregate<InvoiceForReport>(aggr).toArray();
