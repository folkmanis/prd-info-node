...
export class MaterialsDaoService implements EntityDao<Material> {
    ...
    constructor(
        @Inject(MATERIALS_COLLECTION) private readonly collection: Collection<Material>
    ) { }

    async findAll({ start, limit, filter }: FilterType<Material>): Promise<Partial<Material>[]> {
    ...
    async insertOne(material: CreateMaterialDto): Promise<Material | undefined> {
    ...
    async updateOne(id: ObjectId, material: UpdateMaterialDto): Promise<Material | undefined> {
    ...
    async validationData<K extends keyof Material>(key: K): Promise<Material[K][]> {
        return this.collection.find(
            {},
            {
                projection: {
