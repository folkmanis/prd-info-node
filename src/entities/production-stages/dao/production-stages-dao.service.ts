...
export class ProductionStagesDaoService implements EntityDao<ProductionStage> {
    ...
    constructor(
        @Inject(PRODUCTION_STAGES_COLLECTION) private readonly collection: Collection<ProductionStage>
    ) { }

    async findAll({ limit, start, filter }: FilterType<ProductionStage>): Promise<Partial<ProductionStage>[]> {
    ...
    async validationData<K extends keyof ProductionStage>(key: K): Promise<ProductionStage[K][]> {
        return this.collection.find()
            .project({ _id: 0, [key]: 1 })
            .map(data => data[key])
            .toArray();
