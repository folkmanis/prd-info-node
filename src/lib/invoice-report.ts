import { DocumentDefinition, Table, Columns, Txt, Cell } from 'pdfmake-wrapper/server';
import Pdfmake from 'pdfmake';
import { InvoiceProduct, Invoice, Job, JobBase, JobProduct, Product, InvoiceForReport } from '../interfaces';
...
export class InvoiceReport {
    ...
    open() {
        const title = (this.invoice.customerInfo?.financial?.clientName || this.invoice.customer) + (this.invoice.invoiceId ? ` / ${this.invoice.invoiceId}` : '');
        this.pdf.add(
        ...
        this.pdf.add(
            new Table([
                ...
                [
                    ...
                    new Cell(new Txt(`${this.invoice.total?.toFixed(2) || 0} EUR`).bold().end).alignment('right').end,
                ]
            ])
