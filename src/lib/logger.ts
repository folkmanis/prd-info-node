...
export default class Logger {
  ...
  static log(level: LogLevels, message: string, metadata?: any) {
    ...
    for (const transp of Logger.transports) {
      // iet cauri izvades kanāliem
      if (level > transp.minlevel) {
        continue;
      } // ja nav līmenis, tad neko nedara
...
export class Console implements Transport {
  minlevel: LogLevels =
    process.env.NODE_ENV === 'development' ? LogLevels.SILLY : LogLevels.ERROR; // console raksta visu
