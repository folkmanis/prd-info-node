...
export class JobsController {
  ...
  private async jobsImport(req: Request, res: Response) {
    ...
    response.insertedCustomers =
      (await this.customersDao.insertCustomers(data.customers)).insertedCount ||
      0;
    response.insertedProducts =
      (await this.productsDao.insertNewProducts(data.products)).insertedCount ||
      0;
    response.insertedPrices =
      (await this.productsDao.addPrices(data.prices)).insertedCount || 0;
    response.insertedJobs =
      (await this.jobsDao.insertJobs(data.jobs)).insertedCount || 0;
  ...
  private async uploadFile(req: Request, res: Response) {
    ...
    busboy.on('finish', async () => {
      ...
      this.jobsDao.updateJob(job.jobId, {
        files: {
          path,
          fileNames,
        },
  ...
  async updateJobs(req: Request, res: Response) {
    ...
    jobs.forEach((job) => {
      ...
      if (
        ...
        typeof job.productsIdx !== 'number'
      ) {
        const err =
          'Invalid data: products must contain entire array or must be one element with index provided';
        req.log.error(err, job);
        throw new Error(err);
      }
    });
  ...
  private async addFolderPathToJob<T extends Partial<Job>>(
    jobId: number,
    job: T,
  ): Promise<T & Pick<Job, 'files'>> {
    const jb = await this.jobsDao.getJob(jobId);
    if (!jb) {
      throw 'No Job';
    }
    const { code } = (await this.customersDao.getCustomer(
      jb.customer,
    )) as Customer;
  ...
  private async newJob(req: Request, res: Response) {
    ...
    if (job instanceof Array) {
      let ids =
        (await this.countersDao.getNextId('lastJobId', job.length)) -
        job.length;
      res.json(
      ...
      if (req.query.createFolder === 'true') {
        ...
        await this.jobsDao.updateJob(jobId, jobPath);
      }
      res.json({
        error: false,
        insertedId: jobId,
  ...
  private async getJobs(req: Request, res: Response) {
    res.json(await this.jobsDao.getJobs(req.query as JobQueryFilter));
  }
}
