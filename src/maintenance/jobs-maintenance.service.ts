...
export class JobsMaintenanceService {
  private logger = new Logger('Jobs Maintenance');
  ...
  private async createCollection(): Promise<void> {
    try {
      await this.client.db().createCollection(JOBS_COLLECTION_NAME, {
        validator: {
  ...
  private async correctFolderPath(base: string[]) {
    ...
    const count = await this.jobsCollection.countDocuments(filter);

    this.logger.log(`Found incorrect pathnames in ${count} jobs`);

    const result = await this.jobsCollection.updateMany(filter, [
      {
        $set: {
          'files.path': {
            $slice: ['$files.path', base.length, { $size: '$files.path' }],
          },
        },
      },
    ]);
